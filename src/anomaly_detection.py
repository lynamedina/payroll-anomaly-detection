...
def load_payroll_data(path: str = PAYROLL_PATH):
    """
    Loads the preprocessed payroll dataset and splits into features and labels.

    Feature columns (X):
        country, currency, role, base_salary, bonus, tax, social_security, net_pay

    Label column (y):
        is_anomaly (0 = normal, 1 = anomaly)

    Excluded columns:
        employee_id (identifier, not a feature)
        anomaly_type (multi-class label, not used for binary classification)

    Args:
        path (str): Path to payroll_processed.csv
    ...
    Returns:
        tuple: (X_train, X_test, y_train, y_test, df)
            ...
            df              : Full dataframe (used for saving predictions)
    """
...
def run_all():
    ...
    # Summary
    print_summary([rf_metrics, gb_metrics, ann_metrics, if_metrics])
